...
def decode_segwit_address(hrp: str, addr: str) -> Tuple[Optional[int], Optional[bytes]]:
    ...
    if hrpgot != hrp:
        return None, None
    if data is None or len(data) < 1 or data[0] > 16:
        return None, None
    converted = convertbits(data[1:], 5, 8, False)
    if converted is None or len(converted) < 2 or len(converted) > 40:
        return None, None
    if data[0] == 0 and len(converted) != 20 and len(converted) != 32:
        return None, None
    if data[0] == 1 and len(converted) != 32:
        return None, None
    return data[0], bytes(converted) 