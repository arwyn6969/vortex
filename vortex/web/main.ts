...
let currentMount: (() => void) | null = null;
/** Browser-only entry for an existing host. Importing this module is safe during SSR. */
export function mount(
  ...
  const previousNavigation = document.documentElement.style.getPropertyValue(
    "--navigation-height",
  );
  ...
  function element(id: string): HTMLElement | null {
    return (
      [...app.querySelectorAll<HTMLElement>("[id]")].find(
        (node) => node.id === id,
      ) ?? null
    );
  }
  let announcement = document.getElementById("journey-announcement");
  const ownsAnnouncement = !announcement;
  if (!announcement) {
    announcement = document.createElement("div");
    announcement.id = "journey-announcement";
    announcement.className = "sr-only";
    announcement.setAttribute("role", "status");
    announcement.setAttribute("aria-live", "polite");
    announcement.setAttribute("aria-atomic", "true");
    app.after(announcement);
  }
  ...
  let soundRequest = 0;
  let view: "tree" | "codex" | "journal" | "rares" | "stories" | "atlas" =
    "tree";
  let renderedPageKey = "";
  const readingPlaces = new Map<
    string,
    { scrollY: number; details: string[] }
  >();
  function measureNavigation() {
    const height =
      app.querySelector(".topbar")?.getBoundingClientRect().height ?? 0;
  ...
  async function commit(
    change: (w: World) => World | Promise<World>,
    redraw = true,
  ): Promise<boolean> {
    if (disposed || busy || damaged) return false;
    busy = true;
    const expected = world.revision;
    const task = async () => {
      const current = await loadWorld(localStorage);
      if (disposed) throw new Error("This game view has closed.");
  ...
  async function act(action: Action) {
    ...
    if (!(await commit((w) => transition(w, action), false))) return;
    if (disposed) return;
    ...
    render();
    if (action.type === "walk") focusPanel("location-title");
    if (action.type === "look")
      app
  ...
  function map(s: Seeker | null) {
    ...
    const edge = PATH_LETTERS.map((p) => {
      ...
      const just = !!visited && lastCrossed?.id === p.id;
  ...
  function dialog(s: Seeker | null) {
    ...
    if (modal === "help") {
      ...
      body =
        '<div class="help-list"><p><strong>Walk, Look, Sit.</strong> Walk a connected stream. Look before a rite. Sit to clear darkness and steady your pillars.</p><p><strong>Open the way.</strong> Return along a stream to reveal its meaning. Mercy and Severity at 25% each at Vibe Temple open Crown. Look at Boundaries to reveal Qoph.</p><p><strong>Take root.</strong> Make a sigil at Meme Studio, visit six temples and complete four rites. Then Root at Kingdom. No wallet needed.</p><p><strong>Bring stories home.</strong> After a rite, a Rare Pepe may send you on an errand. Return three stories and reveal four streams to celebrate Chapter II at Kingdom.</p><p><strong>Follow your curiosity.</strong> Stories tracks an errand. The Living Atlas connects ideas and points you toward encounters on this same tree.</p></div>';
  ...
  function render() {
    ...
    const openDetails = seekerChanged
      ? []
      : [...app.querySelectorAll<HTMLDetailsElement>("details[open]")].map(
          (el) =>
    ...
    app.innerHTML =
      ...
      (waitingWorker
        ...
        : "") +
      dialog(s);
    // A feedback animation belongs to one successful action, not each rerender.
    lastFeel = null;
    measureNavigation();
    navigationObserver?.disconnect();
    const topbar = app.querySelector(".topbar");
    if (topbar) navigationObserver?.observe(topbar);
    const d = app.querySelector("dialog");
    if (d) {
      d.showModal();
      listen(d, "cancel", (event: Event) => {
    ...
    for (const id of rememberedDetails) {
      const el =
        id === "ask-guide"
          ? app.querySelector<HTMLDetailsElement>(".ask-guide")
          : (element(id) as HTMLDetailsElement | null);
      if (el?.tagName === "DETAILS") el.open = true;
    }
    const restore = focused
      ? element(focused)
      : focusData.length
        ? [...app.querySelectorAll<HTMLElement>("button,a,[data-go]")].find(
            (el) =>
  ...
  async function toggleSound() {
    ...
    try {
      ...
      const available = await setFeelEnabled(on);
  ...
  listen(app, "click", async (event: MouseEvent) => {
    ...
    if ("atlasEntry" in d && s && atlasEntry(d.atlasEntry!)) {
      ...
      if (atlasState.compare.includes(d.compare!))
        atlasState.compare = atlasState.compare.filter(
          (id) => id !== d.compare,
        );
      else if (atlasState.compare.length < 3)
        atlasState.compare.push(d.compare!);
      else {
        notice(
          "Compare up to three entries. Remove one before adding another.",
        );
        return;
      }
      render();
      ...
      try {
        ...
        listen(worker, "statechange", returnToGame);
        returnToGame();
      ...
      } else if (hint.panel === "sigil") element("sigil")?.focus();
      else
        focusPanel(
          hint.panel === "inquiry"
            ? "inquiry-panel"
      ...
      view = "tree";
      render();
      const target = element("story-" + d.track);
  ...
  listen(app, "keydown", (event: KeyboardEvent) => {
    if (
  ...
  listen(app, "submit", async (event: Event) => {
    event.preventDefault();
    if (busy) return;
    ...
    try {
      ...
      if (form.id === "rename-form")
        await commit((w) => {
          ...
          if (
            n.seekers.some(
              (p) =>
                p.id !== s.id && p.name.toLowerCase() === name.toLowerCase(),
            )
          )
      ...
      if (form.id === "atlas-search") {
        ...
        if (
          matches.length &&
          !matches.some((x) => x.id === atlasState.selected)
        )
  ...
  listen(app, "change", async (event: Event) => {
  ...
  listen(window, "storage", async (event: StorageEvent) => {
      ...
      .then((registration) => {
        ...
        listen(registration, "updatefound", () => {
          if (registration.installing)
            listen(registration.installing, "statechange", check);
        });
      })
      .catch(() => {
  ...
  const disposeJourneyTools = registerJourneyTools(
    ...
    async (action, expected) => {
      if (disposed || loading || busy || damaged || newJourney)
  ...
  function unmount() {
    ...
    if (ownsAnnouncement) announcement?.remove();
    ...
    else document.documentElement.style.removeProperty("--navigation-height");
    if (currentMount === unmount) currentMount = null;
  }
