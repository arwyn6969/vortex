...
from src.core.crypto.wallet import BitcoinWallet
from src.core.crypto.keys import is_message_safe

def create_parser() -> argparse.ArgumentParser:
    """Create argument parser with all options."""
    parser = argparse.ArgumentParser(description="Sign messages with Bitcoin private keys")
    
    # Support both new and old argument styles for backward compatibility
    parser.add_argument("--private-key", "-k", dest="private_key",
                       help="Bitcoin private key (WIF or hex format)")
    parser.add_argument("private_key_pos", nargs="?",
                       help="Bitcoin private key (positional argument for backward compatibility)")
    parser.add_argument("--message", "-m", dest="message",
                       help="Message to sign")
    parser.add_argument("message_pos", nargs="?",
                       help="Message to sign (positional argument for backward compatibility)")
...
def main() -> None:
    ...
    try:
        ...
        private_key = args.private_key or args.private_key_pos
        ...
        is_safe, error = is_message_safe(message)
        if not is_safe:
        ...
        # Get wallet info
        wallet_info, error = wallet.process_wallet_input(private_key)
        if error:
            result['error'] = error
            format_output(result, args.output, args.qr)
            return
        ...
        # Sign message
        signature, error = wallet.sign_message(message, private_key)
        if error:
            result['error'] = error
            format_output(result, args.output, args.qr)
            return
